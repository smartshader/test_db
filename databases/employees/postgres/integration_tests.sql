-- PostgreSQL Employees Database Integration Tests
-- This file contains SQL statements to validate that the database was loaded correctly

-- Test 1: Database connection works
SELECT 'Database connection' as test_name, 
       CASE WHEN current_database() = 'test_db' THEN 'PASS' ELSE 'FAIL' END as result;
...
SELECT 'Dept-employee assignments count' as test_name,
       CASE WHEN COUNT(*) = 331603 THEN 'PASS'
            ELSE 'FAIL - Expected 331603, got ' || COUNT(*) END as result
FROM dept_emp;
...
SELECT 'Dept-manager assignments count' as test_name,
       CASE WHEN COUNT(*) = 24 THEN 'PASS'
            ELSE 'FAIL - Expected 24, got ' || COUNT(*) END as result
FROM dept_manager;
